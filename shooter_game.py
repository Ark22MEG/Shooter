...
class Player(GameSprite):
    def update(self):
        ...
        if keys[K_RIGHT] and self.rect.x < width - 85:
            self.rect.x += self.speed
    def fire(self):
        bullet = Bullet(img_bullet, self.rect.centerx + 6, self.rect.top, -15, 15, 20)
        bullets.add(bullet)
...
for i in range(3):
    asteroid = Enemy(img_enemy2, randint(0, width - 80), -40, randint(1, 7), 50, 50 )
    ...
    if finish != True:
        ...
        if reload_bullets:
            ...
            if now_time - last_time < 3:
                reload_text = font1.render('Ты подождать не можешь بثف ', True, (255,0,0))
        ...
        life_text = font1.render(str(life), True, life_color)
